...
function whitewash_int {
	...
	BB_ENV_EXTRAWHITE="$BB_ENV_EXTRAWHITE SKIP_GENERATE_AUDIO"
...
test -d ${BUILD_DIR} &&
	...
	echo "GIT_SERVER ?= \"git@gitlab.com/vivint/horizontals/EM\"" >> ${BUILD_DIR}/conf/local.conf &&
    ...
    sed -i -e '/BB_NUMBER_THREADS/d' ./conf/local.conf &&
