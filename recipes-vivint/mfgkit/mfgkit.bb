...
do_compile() {
	...
	f=$(latest 'slimline-qt5-image.*image.emmc.zip$' ${DEPLOY_DIR_IMAGE})
	cp ${f} ${ddir}/emmcimage
	...
	#   u-boot
	#   nofec version of dtb
	#   special reflash u-boot boot script
	#   
	# all the files we need are relative symlinks so cp with -L will convert
	# the links to files
	tar  xvf ${DEPLOY_DIR_IMAGE}/slimline-qt5-image-imx6dl-slimline.tar.gz -C ${tmpdir} --wildcards ./boot/
	...
	cp ${DEPLOY_DIR_IMAGE}/slimline-qt5-image-imx6dl-slimline.tar.gz ${ddir}/mfgtool/Profiles/Linux/OS\ Firmware/files/
