...
if [ x"$TARGET_VERSION" == x"nightly" -a "$NIGHTLY_DATE" ]; then
    RUST_BIN_RECIPE="$ROOT_DIR/rust-bin-cross_${TARGET_VERSION}-${NIGHTLY_DATE}.bb"
    CARGO_BIN_RECIPE="$ROOT_DIR/cargo-bin-cross_${TARGET_VERSION}-${NIGHTLY_DATE}.bb"
else
    RUST_BIN_RECIPE="$ROOT_DIR/rust-bin-cross_$TARGET_VERSION.bb"
    CARGO_BIN_RECIPE="$ROOT_DIR/cargo-bin-cross_$TARGET_VERSION.bb"
fi
